...
const enrichFileMetadata = (uploadFile) => {
	const originPath = uploadFile?.originFileObj?.path || uploadFile?.path;
	...
	if (!enhancedFile.originalName) {
		const derivedOriginalName =
			uploadFile?.originFileObj?.name || uploadFile?.name || enhancedFile.name;
...
export function Dragger() {
	...
	const onChange = (info) => {
		...
		if (status === "done") {
			const originPath = info.file?.originFileObj?.path || info.file?.path;
	...
	useEffect(() => {
		...
		const needsFolderPath = files.some(
			(file) => !file.folderPath && (file?.originFileObj?.path || file?.path),
		);
		if (needsFolderPath) {
			setFiles(
				files.map((file) =>
	...
	const generateTiffPreview = (file, callback) => {
		...
		reader.onload = function (event) {
			try {
				...
				if (!firstPage.t256 || !firstPage.t257)
					throw new Error(
						"First page is missing essential tags (width and height)",
					);
				...
				if (width > 0 && height > 0) {
					...
					console.error("TIFF image has invalid dimensions:", {
						width,
						height,
					});
	...
	const handlePreview = async (file) => {
		...
		if (targetFile?.folderPath) {
			...
			const originPath = file?.originFileObj?.path || file?.path;
			setPreviewFileFolderPath(
