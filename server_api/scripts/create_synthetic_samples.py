...
def create_synthetic_volume(output_path, shape=(165, 1024, 768), is_label=False):
    ...
    if is_label:
        ...
        for i in range(1, num_mitochondria + 1):
            ...
            # Create ellipsoid
            for z in range(
                max(0, z_center - z_radius), min(shape[0], z_center + z_radius)
            ):
                for y in range(
                    max(0, y_center - y_radius), min(shape[1], y_center + y_radius)
                ):
                    for x in range(
                        max(0, x_center - x_radius), min(shape[2], x_center + x_radius)
                    ):
                        # Ellipsoid equation
                        if (
                            (z - z_center) ** 2 / z_radius**2
                            + (y - y_center) ** 2 / y_radius**2
                            + (x - x_center) ** 2 / x_radius**2
                        ) <= 1:
    ...
    images[0].save(
        output_path, save_all=True, append_images=images[1:], compression="tiff_deflate"
    )
...
if __name__ == "__main__":
    ...
    create_synthetic_volume(
        "samples_pytc/lucchiLabels.tif", shape=(50, 512, 512), is_label=True
    )
