...
from server_api.auth import models, database
...
def get_neuroglancer_url(
    ...
    try:
        ...
        ng_state = {
            "dimensions": {"x": [5e-9, "m"], "y": [5e-9, "m"], "z": [5e-9, "m"]},
        ...
        return {"url": viewer_url, "message": "Viewer ready (requires NIfTI files)"}
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Error preparing Neuroglancer URL: {str(e)}"
        )
...
def get_synapses(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status: error, correct, incorrect, unsure"
    ),
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(database.get_db),
):
